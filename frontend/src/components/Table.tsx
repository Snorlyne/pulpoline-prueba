...
export default function Table({ data }: TableProps) {
  ...
  const handleFavorite = async (city: string) => {
    if (!user) {
      ...
      try {
        if (!favorite) {
          const { message } = await addFavorite(city, user);
          setFavorite(true);
          showToastAlert(message, "success");
        } else {
          const { message } = await removeFavorite(city, user);
          setFavorite(false);
          showToastAlert(message, "success");
        }
      } catch (error) {
          ...
          {data ? (
            <tbody>
              <tr className="bg-white dark:bg-gray-800 dark:border-gray-700 border-b">
                ...
                <td className="px-6 py-4">{data?.current.condition.text}</td>
                <td className="px-6 py-4">
                  <div>
                ...
                <td className="px-6 py-4">{data?.current.humidity}%</td>
                <td className="px-6 py-4">{data?.location.localtime}</td>
                <td className="px-6 py-4">
